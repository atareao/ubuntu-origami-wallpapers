...
for i in $parentdir/scalable/*.svg
do
	src=$i
	dest=$(basename $i)
	destfile=${dest/.svg/.png}
	for j in $(ls -d $parentdir/wallpapers/*)
	do
		if [ "$j" != '.' ] && [ "$j" != '..' ]
		then
			res=$(basename $j)
			width=${res/x*/}
			height=${res/*x/}
			dest=$parentdir/wallpapers/$res/$destfile
			echo ""
			echo "Exporting $destfile (as ${width}x${height})"
			echo "---------------------------------------"
			if [[ $execute == "inkscape" ]]
			then
				inkscape "$src" --export-filename="$dest" --export-type=png --export-width=$width --export-height=$height
